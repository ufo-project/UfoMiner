...
volatile enum sha_algos opt_algo = ALGO_X17R;
int opt_n_threads = 0;
int gpu_threads = 1;
...
// get const array size (defined in UfoMiner.cpp)
int options_count()
{
	int n = 0;
...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	/* Assemble block header */
	memset(work->data, 0, sizeof(work->data));
	if (opt_algo == ALGO_X17R) {
		// first 4 bytes
		memset((unsigned char*)&work->data[0], 0x0, 4);
		// from 5th to 36th bytes
		memcpy((unsigned char*)&work->data[1], sctx->job.prevhash, 32);
		// from 37th to 40th bytes
		memset((unsigned char*)&work->data[9], 0x0, 4);
		// from 41st to 72nd bytes
		memcpy((unsigned char*)&work->data[10], sctx->job.input, 32);
		...
		applog(LOG_WARNING, "stratum_gen_work: opt_algo is not X17R");
		return false;
	}
...
static void *miner_thread(void *userdata)
{
	...
	while (!abort_flag) {
		...
		switch (opt_algo) {
		case ALGO_X17R:
			rc = scanhash_x17r(thr_id, &work, max_nonce, &hashes_done);
			break;
		default:
			/* should never happen */
			goto out;
...
int main(int argc, char *argv[])
{
	...
	printf("*** UfoMiner " PACKAGE_VERSION " for nVidia GPUs by tpruvot@github ***\n");
	printf("***       Modified by Ufo Community Team       ***\n");
	if (!opt_quiet) {
